                                ...
                                @foreach ($jamList as $index => $data)
                                    <tr id="row_{{ $loop->parent->iteration }}_{{ $loop->iteration }}">
                                        ...
                                        <td style="text-align:center; vertical-align:middle;">
                                            ...
                                            @endif
                                            @if (!$foundAksi)
                                                <button id="tambahMapelButton" type="button" class="btn btn-primary btn-sm" data-toggle="modal" data-target="#myModal" data-hari="{{ $data->hari }}" data-jam_ke="{{ $data->jam_ke }}" data-id_jam="{{ $data->id_jam }}" data-row="{{ $loop->iteration }}">
...
$('#editMapelButton').on('click', function() {
    console.log("tes 3");
        ...
        if (id_mapel !== '' && id_guru !== '' && id_kelas !== '') {
            $.ajax({
                ...
                data: {
                    ...
                    id_guru: id_guru,
                    id_jam: id_jam,
                    id_kelas: id_kelas, // Memastikan id_kelas disertakan dalam data
...
$('#editChangesBtn').click(function () {
    ...
    if (idMapel !== '' && idGuru !== '' && idKelas !== '') {
        $.ajax({
            type: 'PUT',
            url: "{{ route('jadwal.update', ':id_jadwal') }}".replace(':id_jadwal', id_jadwal),
            data: {
                id_ta: id_ta,
                id_mapel: idMapel,
                id_guru: idGuru,
