...
@section('title','Halaman Guru | Kelola Nilai')
@section('content_header')
<h1>Halaman Kelola Nilai,
    <span  class="label label-lg label-light-success label-inline" style="font-weight:bold ;color: #ff9e2a; border-radius: 5px; padding: 3px 8px; text-align: center;">{{$murid->nama_murid}}</span>
    <div class="d-flex justify-content-between mt-2">
        <div>
            <h3 class="d-inline-block mt-3">
                Kelas: <button class="btn btn-primary">{{$kelas->nama_kelas}}</button>
            </h3>
        </div>
        <div>
            <h3 class="d-inline-block mt-3">
                Kode TA Kelas: <button class="btn btn-primary">{{$kelas->tahunAjaran->kode_ta}}</button>
            </h3>
        </div>
    </div>
</h1>
...
<div class="row">
    <div class="col-12">
        <div class="mb-3  mt-2">
            <a href="{{ route('modul.daftar-murid', ['id_kelas' => $kelas->id_kelas, 'id_guru' => $guru->id_guru]) }}" class="btn btn-primary">
                        ...
                        <table border="0" width="100%" cellpadding="0" cellspacing="0" style="border-collapse: collapse;">
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>: {{ $murid->nisn }}</td>
                                    <td>Wali Murid</td>
                                    <td>: @if(isset($murid->ortus) && $murid->ortus->isNotEmpty())
...
<!-- Tampilkan jadwal pelajaran dan nilai -->
<div class="row">
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                ...
                <table class="table table-hover table-bordered table-stripped">
                    <thead>
                        <th colspan="5" style="text-align: center">Data Diambil Dari Jadwal Pelajaran</th>
                        ...
                        @else
                            @foreach($jadwalData as $index => $data)
                                <tr>
                                    ...
                                    <form action="{{ route('nilai.destroy-data', $data['id_nilai']) }}" method="POST" style="display: inline;">
                                        @csrf
                                        @method('DELETE')
